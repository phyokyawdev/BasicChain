...
public class Secp256k1 {
    ...
    /**
     * Verify data using the ECDSA algorithm.
     */
    public static boolean verifyTransaction(byte[] message, byte[] sigR, byte[] sigS) {
        try{
            Security.addProvider(new BouncyCastleProvider());
            ECDSASigner signer = new ECDSASigner();

            BigInteger r = new BigInteger(1, sigR);
            BigInteger s = new BigInteger(1, sigS);
    ...
    public static byte[] recoverPublicKey(byte[] sigR, byte[] sigS, byte[] sigV, byte[] message) {
        ...
        try {
            ...
            System.out.println("Error recovering public key from message");
        }
